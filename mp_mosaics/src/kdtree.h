...
class KDTree
{
  ...
  public:
    ...
    typename KDTree<Dim>::KDTreeNode* treeHelper(vector<Point<Dim>>& newPoints, int partDim);

    typename KDTree<Dim>::KDTreeNode * findNearestNeighbor(KDTreeNode * const &  subtree, Point<Dim> const & target, 
                                            int partDim) const;
