...
void DHHashTable<K, V>::remove(K const& key)
{
    ...
    while (i < size) {
        unsigned index = (h + i * step) % size;
        if (should_probe[index] && table[index] != nullptr && table[index]->first == key) {
            table[index]->first = K(); // mark as tombstone
            table[index]->second = V();
            break;
        } else if (!should_probe[index] && table[index] == nullptr) {
            break;
        }
        i++;
    }
