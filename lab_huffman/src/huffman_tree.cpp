...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (!singleQueue.empty() || mergeQueue.size() > 1) {
        TreeNode* firstSmallestNode = nullptr;
        TreeNode* secondSmallestNode = nullptr;

        for (int i = 0; i < 2; ++i) {
            if (singleQueue.empty()) {
                if (firstSmallestNode == nullptr) {
                    firstSmallestNode = mergeQueue.front();
                } else {
                    secondSmallestNode = mergeQueue.front();
                }
                mergeQueue.pop();
            } else if (mergeQueue.empty()) {
                if (firstSmallestNode == nullptr) {
                    firstSmallestNode = singleQueue.front();
                } else {
                    secondSmallestNode = singleQueue.front();
                }
                singleQueue.pop();
        ...
        TreeNode* combinedNode = new TreeNode(firstSmallestNode->freq.getFrequency() + secondSmallestNode->freq.getFrequency());
        combinedNode->left = firstSmallestNode;
        combinedNode->right = secondSmallestNode;

        mergeQueue.push(combinedNode);
    }
