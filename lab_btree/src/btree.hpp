...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    auto c_insert = child->children.end();
    auto e_insert = child->elements.end();
    new_right->children.assign(mid_child_itr, c_insert);
    new_right->elements.assign(mid_elem_itr + 1, e_insert);
